...
export const useNews = (isFollowing: boolean) => {
    ...
    try {
      ...
      if (followedUniversities.length > 0) {
        const newsPromises = followedUniversities.map((university: any) =>
          fetchNewsFromUniversity(
            university.url,
            university.image,
            university.id,
            university.miniature
          )
        );
  ...
  const fetchNewsFromUniversity = async (
    url: string,
    universityImage: string,
    universityId: string,
    universityMiniature: string
  ) => {
    try {
      const response = await axios.get(
        `${BASE_URL}/npm/${encodeURIComponent(url)}`,
        {
          params: { page, limit },
        }
      );
      ...
      if (response.data && response.data.items) {
        return response.data.items.map((item: any) => {
          const { imageUrl, cleanedDescription } = extractImageFromDescription(
            item.description
          );
          return {
            ...item,
        ...
        console.error(
          "Unexpected response structure or null data:",
          response.data
        );
        Alert.alert(
          "Erro",
          "A resposta do servidor não é a esperada ou está vazia."
        );
        return [];
      }
    } catch (error) {
  ...
  const handleRemoveNews = async (news: any) => {
    if (!user) {
      Alert.alert(
        "Erro",
        "Você precisa estar logado para remover uma notícia."
      );
      return;
    }
    ...
    try {
      const response = await axios.delete(`${BASE_URL}/remove-news`, {
        data: { userId: user.id, newsUrl: news.link },
      });
